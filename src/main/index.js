...
app.whenReady().then(() => {
  ...
  // Apply Cloudflare account ID from settings
...
ipcMain.handle('deploy:start', () => {
  ...
  deployer
    .deploy(project.path, project.name, (evt) => {
      safeSend('deploy:event', evt);
    })
    .catch((err) => {
      safeSend('deploy:event', { type: 'error', message: err.message });
    });
...
ipcMain.handle('deploy:status', async () => {
  ...
  // Auto-detect Cloudflare account ID on first use if not set
  if (!deployer.accountId) {
    const settings = getSettings();
    if (!settings.cloudflareAccountId) {
      const detectedId = await deployer.autoDetectAccountId();
      if (detectedId) {
        deployer.setAccountId(detectedId);
        settings.cloudflareAccountId = detectedId;
        saveSettings(settings);
      }
    }
  }

  const projSettings = projectManager.getProjectSettings(project.name);
  return deployer.getDeployedUrl(project.name, projSettings.deployProjectName);
});
...
ipcMain.handle('attachment:save', (_event, projectName, name, dataUrl) => {
  const project = projectManager.getProjectByName(projectName);
  if (!project) throw new Error('No active project');
  const attachDir = path.join(project.path, '.webgen', 'attachments');
  ...
  return { name: safeName, path: filePath, relativePath: `.webgen/attachments/${safeName}` };
});
