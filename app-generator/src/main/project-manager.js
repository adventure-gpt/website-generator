...
class ProjectManager {
  ...
  detectWorkspace() {
    ...
    const candidates = [
      path.join(home, 'Documents', 'desktop-apps'),
      path.join(home, 'desktop-apps'),
    ];
    for (const dir of candidates) {
      if (fs.existsSync(dir)) return dir;
    }
    // Default: create in Documents/desktop-apps
    return candidates[0];
  }
  ...
  _copyTemplates(projectDir) {
    // Templates are bundled in the app's resources or in the app-generator/templates dir
    const possibleTemplateDirs = [
      path.join(__dirname, '..', '..', '..', 'templates'),             // dev: app-generator/templates
  ...
  static defaultProjectSettings() {
    return {
      ...
      electronTarget: '',    // e.g. 'win', 'mac', 'linux' — empty means auto-detect
    };
  }
  ...
  getProjectSettings(name) {
    ...
    const settingsFile = path.join(project.path, '.appgen', 'settings.json');
    try {
  ...
  setProjectSettings(name, settings) {
    ...
    const appgenDir = path.join(project.path, '.appgen');
    if (!fs.existsSync(appgenDir)) {
      fs.mkdirSync(appgenDir, { recursive: true });
    }

    const settingsFile = path.join(appgenDir, 'settings.json');
    let existing = {};
    try {
