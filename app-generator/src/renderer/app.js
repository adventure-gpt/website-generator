...
function setupAPIListeners() {
  window.api.onAIEvent(function (event) {
    ...
    console.log('AI event:', event.type, evtProject, event.text ? event.text.substring(0, 80) : '');
    ...
    if (event.type === 'done') {
      renderMessages();
      restoreProjectUI(ps);
      // Auto-launch the app after AI finishes building/modifying
      if (isActive && evtProject) {
...
async function selectProject(name) {
  ...
  try {
    await window.api.getProjectSettings(name);
  ...
  // Scan recent messages for distribution URLs before rendering
  var recentMsgs = (state.messages[name] || []).slice(-15);
  for (var ri = 0; ri < recentMsgs.length; ri++) {
    if (recentMsgs[ri].role === 'assistant' && recentMsgs[ri].content) {
      var c = recentMsgs[ri].content;
      // GitHub release URL
      if (!ps.deployedUrl) {
        var ghMatch = c.match(/https:\/\/github\.com\/[^\s),]+\/releases[^\s),]*/i);
        if (ghMatch) ps.deployedUrl = ghMatch[0].replace(/[.,!;:]+$/, '');
      }
      // Landing page URL (pages.dev)
      if (!ps.landingUrl) {
        var pgMatch = c.match(/https:\/\/[a-z0-9][-a-z0-9]*\.pages\.dev/i);
  ...
  renderProjectList();
  checkDevServer();
  updateDistributeButton(aps().deployedUrl || null);
...
function createWelcome() {
  var div = el('div', { className: 'chat-welcome', id: 'chat-welcome' }, [
    ...
    el('p', { textContent: 'Describe an app in plain English and the AI will build it for you.' }),
  ]);
  var chipsEl = el('div', { className: 'prompt-chips' });
  var examples = [
    { label: 'Note-taking app', prompt: 'A markdown note-taking app with a file browser sidebar and local file storage' },
    { label: 'Finance tracker', prompt: 'A personal finance tracker that imports CSV bank statements and shows spending charts' },
    { label: 'Clipboard manager', prompt: 'A clipboard manager that runs in the system tray and saves clipboard history' },
    { label: 'Image tool', prompt: 'A local image resizer and converter with drag-and-drop and batch processing' },
  ];
...
async function sendMessage() {
  ...
  var safetyTimer = setInterval(function () {
    ...
    if (Date.now() - (ps._lastEventTime || 0) > 300000) {
